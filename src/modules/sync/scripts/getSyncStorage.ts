...
export default async function getSyncStorage<T>(key: string){
    try {
        ...
        if(!!storageValues && storageValues.length > 0){
            if(storageValues.some(f => f.sync)){
                const newStorageValues = storageValues.filter(f => !f.sync)
                if(!!newStorageValues && newStorageValues.length > 0){
                    await storage.setItem(key, newStorageValues)
                    return newStorageValues
                }
            }
