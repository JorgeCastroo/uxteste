...
export default async function send(dispatch: Function, network: boolean, redirect: () => void, openSuccess: () => void, idMotorista: number, idLista: number, volumes: number[]){
    try {
        if(network) await saveLista(dispatch, openSuccess, false, idMotorista, idLista, volumes)
        else{
