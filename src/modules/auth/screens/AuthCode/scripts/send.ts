...
async function getToken() {
    ...
    const body = 'username=admin&password=ux@solutions&scope=portal&client_id=TruxDiscoveryApi&client_secret=4531a3f2b670e434344345718aa8c14976&grant_type=password'
...
async function getConfiguracao(codigoAcesso: any, token: TruxDiscoveryLogin) {
    ...
    const response = await fetch(`${serverApi}/api/configuracao/parametros?codigoTransportador=${codigoAcesso}`, {
        method: 'GET',
        headers: newHeaders
    })
...
export default async function send(navigation: any, dispatch: Function, body: typeof codeFormValues) {
    ...
    try {
        ...
        dispatch(R.setRequestSendAuthCodeMessage("O código de transportadora informado é inválido"))
        console.log(error);
